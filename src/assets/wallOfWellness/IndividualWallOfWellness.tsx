...
const IndividualWallOfWellness = () => {
  const [wowStory, setWoWStory] = useState<IndividualWowStory | null>(null)

  const { id } = useParams()
  const [files, setFiles] = useState([] as any)

  const fetchWoWStories = async ({ token }: { token: string }) => {
    try {
  ...
  const [userData, setUserData] = useState({
    ...
    userId: null,
    imageUrl: null,
    accessToken: 'b6cc3d009218a354f208fe97f15ac08ac102f58e435b0a11d05d271d9a92d475e2393626d26899699a15722c2f29a40d',
  })
  ...
  useEffect(() => {
    ;(window as any).receiveUserData = ({ data }, { data: String }) => {
    ...
    return () => {
      delete (window as any).receiveUserData
    }
  }, [])
      ...
      <div className="mx-auto max-w-[400px] bg-[#24262B]">
        ...
        <div className="pt[17px] px-[24px] mt-[17px] pb-[4px]">
          ...
          <div className="mt-[10px]">
            <img src={calendar} alt="Calendar" className="h-[20px] w-[20px] float-left mr-[10px]" />
        ...
        <div className="mt-[36px] pb-[100px] p-[28px] bg-white rounded-t-[16px]">
          ...
          <div className="mt-[20px]">
            <p className="text-[#000000] font-medium leading-[24px]">
              <span className="text-[24px] leading-[38.4px]">{wowStory?.userStory.content?.charAt(0)}</span>
              <span className="text-[14px] leading-[17.5px]">{wowStory?.userStory.content?.slice(1)}</span>
            </p>
          </div>
        </div>
  ...
  // : { title: string; subtitle: String }
  {
    return (
      <div className={`${title == 'Medal' ? 'mt-[32px]' : 'mt-[22px]'}`}>
        <p className="text-[10px] text-[#00000099] font-medium leading-[12.5px]">{title}</p>
        <p className="text-[12px] text-[#000000CC] font-medium leading-[15px]">{subtitle}</p>
      </div>
    )
  }
const FooterAppDowwnload = () => {
  const handlePlayStoreClick = () => {
    window.open('https://play.google.com/store/apps/details?id=com.koshiqa', '_blank')
  }
  ...
  return (
    <div className="bg-white max-w-[400px] min-w-[400px]">
      <div
        ...
        <img onClick={handlePlayStoreClick} src={playStore} alt="Play Store" className="h-[42px] " />
        <img onClick={handleAppStoreClick} src={appStore} alt="App Store" className="h-[42px]  ml-3" />
      </div>
    </div>
  )
}
